...
// Server-side fetch function (adapted for client)
async function fetchData() {
  const res = await fetch("https://bgmibackend.onrender.com/tournament", {
    cache: "force-cache", // static generation caching
  });
  return res.json();
}
...
const HomePage = () => {
  ...
  const [extraData, setExtraData] = useState(null); // added for fetchData results

  useEffect(() => {
    const fetchTournaments = async () => {
      try {
        // Fetch original tournaments
        const res = await fetch("https://bgmibackend.onrender.com/tournament");
        if (!res.ok) throw new Error(`❌ Server responded with ${res.status}`);
        const data = await res.json();
        setTournaments(data);

        // Fetch extra data from fetchData
        const extra = await fetchData();
        setExtraData(extra);
      } catch (err) {
        console.error(err);
        ...
        <div className="-mt-8 w-[96vw] lg:w-[55vw] lg:ml-10">
          {loading ? (
            ...
            <>
              ...
              {/* Display the extra data fetched from fetchData */}
              {extraData && (
                <div className="mt-10 w-[90%] bg-gray-100 p-4 rounded-lg">
                  <h2 className="text-lg font-bold mb-2">Extra Data:</h2>
